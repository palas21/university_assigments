...
class gradientDescent():
    ...
    def gradient(self):
        ...
        gradient = np.matmul(self.x.T ,self.predict(self.x) - self.y.reshape(-1, 1)) / self.x.shape[0]
    ...
    def fit(self,lr=None, n_iterations=None,x_train=None,y_train=None):
        k = 0
    ...
    def cross_validate(self, k=5):
        ...
        for i in range(k):
            ...
            x_val = self.x[validation_indices]
            y_val = self.y[validation_indices]
            
            self.w = np.resize(self.w,(x_train.shape[1],1))
            ...
            error = np.mean(np.square(y_pred - y_val)) 
...
class SGD():
    ...
    def gradient(self, i):
        ...
        gradient = self.x[i].reshape(-1,1) * (self.predict(self.x[i].reshape(-1,1),i)-self.y.reshape(-1, 1)[i])
    ...
    def fit(self,lr=None, n_iterations=None):
        ...
        while k < n_iterations:
            ...
            for i in indices:
                ...
                self.w -= self.lr * gradient / self.x.shape[0]
    ...
    def predict(self, x, i = None):
        ...
        if i == None:
            ...
            y_pred = x.dot(self.w.reshape(-1,1)[i])
    ...
    def cross_validate(self, k=5):
        ...
        for i in range(k):
            ...
            self.w = np.resize(self.w,(x_train.shape[1],x_train.shape[0]))
